...
<div x-data="tagihanPembelianShowData()" class="order-page">
  ...
  <div>
    ...
    <div class="order-title-row">
      <h1 class="order-title display">{{ $bill['id'] }}</h1>
      <x-misc.status-badge :status="$bill['status']" />
    </div>
    <div class="order-sub">
      Ref. PO: <strong>{{ $bill['poRef'] }}</strong> &nbsp;·&nbsp;
      Diterbitkan {{ $bill['tanggal'] }} &nbsp;·&nbsp;
      Jatuh Tempo {{ $bill['jatuhTempo'] }}
    </div>
  </div>
  ...
  <div class="card card-bd--form">
    ...
    <div class="order-form-grid-3">
      <x-misc.field label="Vendor">
        <div class="input input--readonly" style="display:flex; align-items:center; gap:10px;">
          <x-misc.avatar :name="$bill['vendor']" />
          <span style="flex:1; font-weight:500;">{{ $bill['vendor'] }}</span>
        </div>
      </x-misc.field>
      <x-misc.field label="No. Tagihan">
        <div class="input mono input--readonly" style="display:flex; align-items:center;">
          <span style="flex:1; font-weight:600;">{{ $bill['id'] }}</span>
        </div>
      </x-misc.field>
      <x-misc.field label="Ref. Purchase Order">
        <div class="input mono input--readonly" style="display:flex; align-items:center;">
          <a href="{{ route('pembelian.show', $bill['poRef']) }}" class="link orange-link" style="flex:1; font-weight:600;">{{ $bill['poRef'] }}</a>
        </div>
      </x-misc.field>
      <x-misc.field label="Tanggal Tagihan">
        <div class="input input--readonly" style="display:flex; align-items:center; gap:8px;">
          ...
          <span style="flex:1;">{{ $bill['tanggal'] }}</span>
        </div>
      </x-misc.field>
      <x-misc.field label="Jatuh Tempo">
        <div class="input input--readonly" style="display:flex; align-items:center; gap:8px;">
          <x-misc.icon name="calendar" :size="14" stroke="var(--ink-4)" />
          <span style="flex:1;">{{ $bill['jatuhTempo'] }}</span>
        </div>
      </x-misc.field>
    </div>
  </div>
  ...
  <div class="card" style="overflow:hidden;">
    ...
    <div class="order-summary" style="margin:0; border-top:1px solid var(--border-line); padding:20px 24px;">
      <div class="order-summary__row">
        ...
        <span class="num" style="font-weight:500;">{{ fmt_rp(collect($poDetailItems)->sum(fn($i) => $i['qty'] * $i['harga'])) }}</span>
      </div>
      <div class="order-summary__divider"></div>
      <div class="order-summary__total">
        <span class="order-summary__total-label">Total Tagihan</span>
        <span class="order-summary__total-value display num">{{ fmt_rp($bill['total']) }}</span>
      </div>
    </div>
  </div>
  ...
  <div style="display:flex; justify-content:flex-end; gap:10px;">
    <a href="{{ route('pembelian.tagihan_list') }}" class="btn btn-ghost">Kembali</a>
    <button class="btn btn-ghost"><x-misc.icon name="print" :size="14" />Cetak Tagihan</button>
