...
@php
    ...
    $editVendor  = collect($kontak)->where('tipe', 'Vendor')->firstWhere('nama', $po['vendor']);
    $editGudang  = collect($gudang)->firstWhere('nama', $po['gudang']);
    $editVendors = collect($kontak)->where('tipe', 'Vendor')->values();
    $editItems   = collect($poDetailItems)->map(function ($i) {
        return ['kode' => $i['kode'], 'nama' => $i['nama'], 'qty' => $i['qty'], 'satuan' => $i['satuan'], 'harga' => $i['harga']];
    })->values();
@endphp

<div x-data="{
  vendor: {{ Js::from($editVendor) }},
  selectedGudang: {{ Js::from($editGudang) }},
  termin: 'Net 14 hari',
  vendors: {{ Js::from($editVendors) }},
  gudangList: {{ Js::from($gudang) }},
  produkList: {{ Js::from($produk) }},
  terminList: ['Net 7 hari', 'Net 14 hari', 'Net 30 hari', 'Net 45 hari', 'COD'],
  items: {{ Js::from($editItems) }},
  ...
  <div>
    ...
    <div style="display:flex; align-items:center; gap:10px;">
      <h1 class="order-title display">Edit Draft PO</h1>
      <x-misc.status-badge status="draft" />
    </div>
    <div class="order-sub">{{ $po['id'] }} · Perubahan hanya tersimpan sebagai draft hingga dikonfirmasi.</div>
  </div>
    ...
    <div class="order-form-grid-4">
      ...
      <x-misc.field label="Pilih Vendor" :required="true">
        <x-misc.select display="vendor ? vendor.nama : 'Pilih Vendor'" hasValue="vendor" placeholder="Cari vendor...">
          <template x-for="v in vendors.filter(v => !q || v.nama.toLowerCase().includes(q.toLowerCase()))" :key="v.id">
          ...
          <template x-if="!vendors.some(v => !q || v.nama.toLowerCase().includes(q.toLowerCase()))">
      ...
      <x-misc.field label="Nomor PO" :required="true">
        <input class="input mono" value="{{ $po['id'] }}" />
      </x-misc.field>

      {{-- Tanggal --}}
      <x-misc.field label="Tanggal" :required="true">
        <input type="date" class="input" value="{{ $toDateInput($po['tanggal']) }}" />
      </x-misc.field>

      {{-- Jatuh Tempo --}}
      <x-misc.field label="Jatuh Tempo" :required="true">
        <input type="date" class="input" value="{{ $toDateInput($po['jatuhTempo']) }}" />
      </x-misc.field>
      ...
      <x-misc.field label="Pembeli">
        <div class="input" style="display:flex; align-items:center; gap:8px;">
          <x-misc.avatar name="Nadia Rahmawati" />
          <span style="flex:1; font-weight:500;">Nadia Rahmawati</span>
        </div>
      </x-misc.field>
    ...
    <div class="order-items-split">
      ...
      <div class="order-summary">
        ...
        <div class="order-summary__row">
          ...
          <span class="num" x-text="fmt(subtotal)"></span>
        </div>
        <div class="order-summary__row">
          <span class="order-summary__label">Diskon</span>
          <span class="num" x-text="'–' + fmt(diskon)"></span>
        </div>
        <div class="order-summary__row">
          <span class="order-summary__label">Est. Ongkir</span>
          <span class="num" x-text="fmt(ongkir)"></span>
        </div>
  ...
  <div class="order-form-footer">
    ...
    <button class="btn btn-primary"><x-misc.icon name="check" :size="14" />Konfirmasi PO</button>
  </div>
