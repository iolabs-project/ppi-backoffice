    ...
    <script>
        function masterPageData() {
            return {
                ...
                editAkunData: { kode: '', nama: '', tipe: '', saldo: 0 },
                openEditAkun(a) {
                    this.editAkunData = { kode: a.kode, nama: a.nama, tipe: a.tipe, saldo: a.saldo };
    ...
    <div x-data="masterPageData()" class="master-page">
        ...
        <div class="master-hd">
            ...
            <div>
                <button class="btn btn-primary" x-on:click="modal = 'add_' + tab">
                    ...
                    <span
                        x-text="{ produk:'Tambah Produk', kontak:'Tambah Kontak', akun:'Tambah Akun', gudang:'Tambah Gudang' }[tab]"></span>
                </button>
            </div>
        </div>
        ...
        <div class="utab">
            @foreach ([['produk', 'Produk'], ['kontak', 'Kontak'], ['akun', 'Chart of Accounts'], ['gudang', 'Gudang']] as [$id, $lbl])
                <button class="utab-item" x-on:click="tab = '{{ $id }}'"
                        ...
                        @foreach ($akunGroups as $prefix => $items)
                            ...
                            @foreach ($items as $a)
                                ...
                                <tr class="row-tap" style="cursor:pointer;"
                                    x-on:click="openEditAkun({ kode: {{ json_encode($a['kode']) }}, nama: {{ json_encode($a['nama']) }}, tipe: {{ json_encode($tipeLbl) }}, saldo: {{ $a['saldo'] }} })">
                                    <td class="mono" style="font-weight:600; font-size:12px; color:var(--ink-4);">
                                        {{ $a['kode'] }}</td>
        ...
        <x-misc.modal title="Edit Akun" show="modal === 'edit_akun'" close-handler="modal = null">
            <div class="form-body">
                ...
                <div class="form-grid-2">
                    ...
                    <x-misc.field label="Saldo">
                        <input class="input num" type="number" style="text-align:right;" x-model="editAkunData.saldo" />
                    </x-misc.field>
                </div>
            </div>
            <x-slot:footer>
