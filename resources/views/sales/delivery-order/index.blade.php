...
@section('content')
    ...
    <div x-data="datatable()" x-init="fetchData()" class="order-page">
        <div class="order-hd">
            <div>
                <h1 class="order-title display">Pengiriman Barang</h1>
        ...
        <div class="table-toolbar">
            <div class="table-search">
                ...
                <input class="table-search__input" placeholder="Cari nomor pengiriman / customer..."
        ...
        function datatable() {
            return {
                ...
                async fetchData() {
                    this.loading = true;
                    try {
                        const response = await axios.get(route('sales.delivery_orders.datatable'), {
                            params: {
                                page: this.page,
                ...
                async fetchSoPickerData() {
                    ...
                    try {
                        ...
                        this.soPickerData = (response.data.data ?? []).filter(so => so.is_deliverable);
                    } catch (error) {
                        Toast.fire({
                            icon: 'error',
                ...
                async handleCreateDeliveryOrder(salesOrderId) {
                    this.closeSoPicker();
                    Swal.fire({
