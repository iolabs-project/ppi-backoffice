                    ...
                    <template x-if="!loading">
                        <template x-for="(row, i) in tableData.data" :key="row.id">
                            <tr class="row-tap" @click="window.location = route('finances.account_payables.show', row.id)">
                                ...
                                <td x-on:click.stop>
                                    <a :href="route('finances.account_payables.show', row.id)" class="btn btn-ghost btn-icon btn-sm" style="border:none;">
            ...
            <div class="form-body">
                <x-misc.field label="Invoice">
                    <x-misc.select
                        ...
                        hasValue="!!pickerSelected" placeholder="Cari nomor invoice / supplier..." min-width="420px" height="40px">
                        <template x-for="inv in pickerInvoices.filter(i => !q || i.number.toLowerCase().includes(q.toLowerCase()) || (i.supplier?.name || '').toLowerCase().includes(q.toLowerCase()))" :key="inv.id">
                            <div class="dropdown-item" @click="pickerSelected = inv; open = false; q = ''">
                                <div style="flex:1; min-width:0;">
                                    ...
                                    <div style="font-size:11.5px; color:var(--ink-4);" x-text="(inv.supplier?.name ?? '-') + ' · Outstanding ' + NumberUtils.formatNumericIntoMask(inv.remaining_amount)"></div>
                                </div>
                            </div>
                        </template>
        ...
        function apDatatable() {
            return {
                ...
                get pickerSelectedLabel() {
                    return this.pickerSelected ? (this.pickerSelected.number + ' - ' + (this.pickerSelected.supplier?.name ?? '-')) : 'Pilih invoice';
                },
