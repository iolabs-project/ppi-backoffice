...
<div class="card" style="overflow:visible;">
    <div class="card-hd">
        <div class="display card-hd-title">Biaya Tambahan</div>
        <button class="btn btn-ghost btn-sm" @click="addCost()">
        ...
        <tbody>
            <template x-for="(cost, i) in formData.costs" :key="i">
                <tr>
                    ...
                    <td>
                        <x-misc.select
                            ...
                            <template
                                x-for="a in accounts.filter(a => !q || a.name.toLowerCase().includes(q.toLowerCase()) || (a.code || '').toLowerCase().includes(q.toLowerCase()))"
                                :key="a.id">
                                <div class="dropdown-item" @click="cost.account_id=a.id; calculate(); open=false; q=''">
                                    <div style="flex:1; min-width:0;">
                                        <div style="font-size:13px;" x-text="a.name"></div>
                                        <div class="mono" style="font-size:11px; color:var(--ink-4);" x-text="a.code">
                                        </div>
                    ...
                    <td style="padding:15px; text-align:center;">
                        <button @click="removeCost(i)" class="btn btn-ghost btn-icon btn-sm" style="border:none;">
                            <x-misc.icon name="trash" :size="14" stroke="var(--ink-4)" />
                        </button>
                    </td>
                </tr>
            ...
            <div class="order-summary__grid">
                ...
                <div class="order-summary__group">
                    ...
                    <div class="order-summary__row">
                        ...
                        <span class="num order-summary__val" x-text="m(costsTotal())"></span>
                    </div>
                </div>
            </div>
